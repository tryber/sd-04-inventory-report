...
class SimpleReport:
    ...
    def generate(self, list_dict):
        ...
        nearest_date = min(not_expired, key=itemgetter("data_de_validade"))[
            "data_de_validade"
        ]
        counter_employees = Counter(
            product["nome_da_empresa"] for product in list_dict
        )
        most_product_employee = max(
            counter_employees, key=counter_employees.get
        )
