...
class SimpleReport:
    ...
    def greater_than_today(self, list_data):
        today = date.today()
        if list_data["data_de_validade"] >= today.strftime("%Y-%m-d"):
            return list_data

    @classmethod
    def generate(self, list_dict):
        oldest_date = min(list_dict, key=itemgetter("data_de_fabricacao"))
        not_expired = list(map(self.greater_than_today, list_dict))
        nearest_date = min(filter(partial(is_not, None), not_expired), key=itemgetter("data_de_validade"))
        return not_expired
